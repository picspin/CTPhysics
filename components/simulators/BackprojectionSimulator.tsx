...
const BackprojectionSimulator: React.FC<Props> = ({ options }) => {
  ...
  useEffect(() => {
    ...
    drawImages();
  }, [selectedImage, projectionCount, fanBeamAngle, generatePhantomData]);
  ...
  // Draw reconstruction
  const drawReconstruction = (ctx: CanvasRenderingContext2D, size: number) => {
