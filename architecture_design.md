...
为与首页 hero 背景保持一致、突出科技质感与简洁强调，采用“深色背景 + 金色强调 + 冷蓝辅助”的方案：

```css
:root {
  /* 主色调（金色光绘） */
  --primary-100: #FFD700; /* Gold */
  --primary-200: #FFAA00; /* Warm Gold */
  --primary-300: #E39A00; /* Deep Gold */

  /* 辅助强调（科技蓝） */
  --accent-100: #3B82F6; /* Blue-500 */
  --accent-200: #1D4ED8; /* Blue-700 */
  --accent-300: #1E40AF; /* Blue-800 */

  /* 文本颜色（深色背景下高对比） */
  --text-100: #F3F4F6; /* Gray-100 */
  --text-200: #D1D5DB; /* Gray-300 */
  --text-300: #9CA3AF; /* Gray-400 */

  /* 背景颜色（与 hero 一致的深色系） */
  --bg-100: rgba(255,255,255,0.06); /* 面板（玻璃态卡片） */
  --bg-200: #050505; /* 主背景（近黑） */
  --bg-300: #111827; /* 次级背景（Gray-900） */
  --bg-400: #1F2937; /* 边框/滚动条背景（Gray-800） */

  /* 边框颜色 */
  --border-100: #374151; /* Gray-700 */
  --border-200: #4B5563; /* Gray-600 */
}
```

说明：
- 页面背景采用与 hero 相同的深色，并在内容区域使用玻璃态卡片（半透明、轻微模糊）保证留白与层次。
- 主强调色使用金色，呈现统一的“光绘”审美；辅助蓝用于链接与图表高亮，保持科技感。
- 文本与子卡片在深色背景下拥有清晰对比，不突兀。
